...
import { getGrid, getV11Pattern } from "@/lib/api";
...
export function CaliMap({ pollutant, horizon }: { pollutant: Pollutant; horizon: Horizon }) {
  ...
  const flyToTarget = usePanelStore((s) => s.flyToTarget);
  ...
  const [pattern, setPattern] = useState<GeoJSON.FeatureCollection | null>(null);
  ...
  useEffect(() => {
    ...
    setPopup(null);
    setGrid(null);
    getGrid(pollutant, horizon)
      .then((data) => {
        if (!abort) setGrid(data as GridFeatureCollection);
      })
      .catch(() => {});
    return () => {
      abort = true;
  ...
  useEffect(() => {
    if (!showModelPattern) return;
    let abort = false;
    getV11Pattern(pollutant)
      .then((data) => {
        if (!abort) setPattern(data);
      })
      .catch(() => {});
    return () => {
      abort = true;
    };
  }, [pollutant, showModelPattern]);

  // Al cambiar contaminante u horizonte, re-consultar el punto activo para que el
  // panel "Radio de consulta" no quede con el valor del gas anterior.
  useEffect(() => {
    const { queryPoint: pt, queryAt: run } = usePanelStore.getState();
    if (pt) void run(pt.lat, pt.lon);
  }, [pollutant, horizon]);

  // Volar al punto cuando llega desde el campo de coordenadas (no desde el click).
  useEffect(() => {
  ...
  const handleMouseMove = (event: MapLayerMouseEvent) => {
    ...
    if (feat.layer.id === "prediction-fill") {
      ...
      setPopup({
        ...
        body: `${formatPlusMinus(value, sigma, 1)} ${POLLUTANT_META[pollutant].unit}`,
  ...
  return (
    ...
    >
      ...
      {queryPoint && (
        <Marker longitude={queryPoint.lon} latitude={queryPoint.lat} anchor="center">
          <span className="block h-3.5 w-3.5 rounded-full border-2 border-white bg-[var(--foreground)] shadow-[0_0_0_2px_rgba(0,0,0,0.35)]" />
        </Marker>
      )}
      ...
      {showModelPattern && pattern && (
        <Source id="v11-pattern" type="geojson" data={pattern}>
          <Layer
            id="v11-pattern-circle"
            type="circle"
            paint={{
              "circle-radius": 6,
              "circle-color": [
