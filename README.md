...
flowchart TD
    ...
    subgraph ETL["ETL Pipeline · Dask (4w × 2t × 4GB)"]
        DL["Parallel Download\nby band / date / tile"]
        CROP["BBox Clip\n[-76.80,-75.81] × [2.62,4.52]"]
        STORE["Azure ADLS Gen2\n306.54 GB · Zarr + GeoTIFF"]
        MANIFEST["manifest.json\nMD5 · 1,507 files · Git"]
    end

    subgraph PREPROC["Preprocessing"]
        RAD["Radiometric correction\nρ=(DN-1000)/10000"]
        NNWCL["NNWCL imputation\nTukey-h + KDTree + Lambert-W"]
        SAM["SAM segmentation\n7,844 segments · 21.6/tile"]
        ALIGN["KDTree temporal alignment\nS2 ↔ S5P ±2 days"]
        CAPTIONS["Enriched captions\ncoords · date · land cover · percentile bracket"]
    end

    subgraph MODEL["LateFusionModel v12 · 18.2M params"]
        GRSCLIP["GeoRSCLIP ViT-B/32\nFROZEN · RS5M pretrained"]
        SAE_IMG["SAEimg\n512→4096→512 · ℓ1 sparse"]
        SAE_TXT["SAEtxt\n512→4096→512 · ℓ1 sparse"]
        GRU["TemporalGRU\n2L · h=256 · win=7"]
        FUSION["Late Fusion MLP\nconcat → 256→128→3"]
    end

    subgraph GEO["Geostatistics · PyKrige + PySAL"]
        VARIO["Spherical variogram\nnugget · sill · range per contaminant"]
        KRIGING["Ordinary Kriging 2D\n25×25 grid · per horizon"]
        MORAN["Moran I + LISA\nautocorrelation certification"]
        KMEANS["K-Means K=4\ntypological pollution profiles"]
    end

    subgraph DEPLOY["Deployment"]
        API["FastAPI + Uvicorn\n/predict · /validate · /metadata"]
        WEB["Next.js + MapLibre + deck.gl\nMapLibre tiles · GeoJSON overlays"]
